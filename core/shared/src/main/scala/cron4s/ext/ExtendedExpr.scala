...
/**
  ...
  */
abstract class ExtendedExpr[E[_ <: CronField] <: Expr[F], F <: CronField, DateTime]
    (expr: E[F])
  ...
  def matchesIn: Predicate[DateTime] = Predicate { dt =>
    ...
    current.map(ev.matches(expr)).getOrElse(false)
  }
  ...
  def step(dateTime: DateTime, step: Int): Option[DateTime] = {
    for {
      ...
      newValue <- ev.step(expr)(current, step).map(_._1)
