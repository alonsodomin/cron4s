...
private[validation] object ops extends NodeValidatorInstances {
  type Validatable[F] = EachNode[F] | AnyNode[F] | ConstNode[F] | BetweenNode[F] | SeveralNode[
    F
  ] | EveryNode[F] | FieldNode[F] | FieldNodeWithAny[F] | Nothing
  def validate[F <: CronField](node: Validatable[F]): List[InvalidField] = node match {
    case field: EachNode[F]    => implicitly[NodeValidator[EachNode[F]]].validate(field)
    case field: AnyNode[F]     => implicitly[NodeValidator[AnyNode[F]]].validate(field)
    case field: ConstNode[F]   => implicitly[NodeValidator[ConstNode[F]]].validate(field)
    case field: BetweenNode[F] => implicitly[NodeValidator[BetweenNode[F]]].validate(field)
